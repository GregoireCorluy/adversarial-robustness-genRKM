...
class Adv_attack_genRKM():
    ...
    def __init__(self, name_model, name_VAE, latent_VAE, VAE_model, oneView):
        ...
        parser.add_argument('--filename', type=str, default=self.name_model, help='Enter Filename')
        ...
        self.net2 = sd_mdl['net2'].double().cpu()
        self.net4 = sd_mdl['net4'].double().cpu()

        self.net1.load_state_dict(sd_mdl['net1_state_dict'])
        self.net3.load_state_dict(sd_mdl['net3_state_dict'])
        self.net2.load_state_dict(sd_mdl['net2_state_dict'])
        self.net4.load_state_dict(sd_mdl['net4_state_dict'])
    ...
    def plot_ori_back_adv_back(self):
        ...
        ax[1,1].set_xticks([])
        ax[1,1].set_yticks([])
        plt.suptitle("Original image, original image reconstructed,\n adversarial image and adversarial image reconstructed")
        plt.show()
    ...
    def plot_distortion(self):
        ...
        plt.title(f'Adversarial attack {type_of_attack} ({len(self.distortion_input)-1} iterations) \nDistortion in the input and output with original image')
